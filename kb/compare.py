...
Datenquellen (kuratiert, ohne Index-Build nutzbar):
  ...
  kb/graph/blockers.json       zusammengefuehrte Blocker-/Fehlermodus-Taxonomie (docs/55)
...
Funktionen:
  ...
  matrix_markdown()              -> Vergleichsmatrix als Markdown (für docs/78)
  failure_map_markdown()         -> Ansatz→Blocker-Landkarte als Markdown (für docs/55)
...
def _data():
    ...
    if _A is None:
        ...
        _F = json.load(open(os.path.join(GRAPH, "blockers.json"), encoding="utf-8"))
    return _A, _F
...
def modes():
    """Fehlermodi in der Form, die dieses Modul erwartet -- gelesen aus der
    ZUSAMMENGEFUEHRTEN Taxonomie kb/graph/blockers.json (docs/55).

    Historie: PR#5 fuehrte eine eigene Datei failure_modes.json mit den IDs
    F1-F15, PR#6 eine blockers.json mit blk-*-IDs. Beim Zusammenfuehren wurde
    blockers.json zur einzigen Quelle; jeder Blocker traegt seine F-ID im Feld
    f_mode weiter, damit die Verweise der aelteren Dokumente aufloesen."""
    out = []
    for b in _data()[1]["blockers"]:
        out.append({
            "id": b.get("f_mode") or b["id"],
            "blocker_id": b["id"],
            ...
            "diagnostic": b.get("diagnosefrage", ""),
            "escape": b.get("fluchtbedingung", ""),
            "tier_divergence": b.get("tier_abweichung"),
            "cases": [d for d in b.get("betrifft", [])],
            "docs": b.get("quelle_docs", []),
            "oracle_test": b.get("orakel_test"),
        })
    return sorted(out, key=lambda m: (m["tier"],
                  int(m["id"][1:]) if m["id"].startswith("F") and m["id"][1:].isdigit() else 99))

def axes():
    return _data()[0]["axes"]

def _mode_index():
    """Index ueber BEIDE Namensraeume: F1-F15 und blk-*."""
    idx = {}
    for m in modes():
        idx[m["id"]] = m
        idx[m["blocker_id"]] = m
...
# ------------------------------------------------- Einbau in die Dokumente
_TARGETS = {
    "matrix": ("docs/78_approach_comparison_matrix.md",
               "<!-- COMPARE-MATRIX:BEGIN (generiert von kb/compare.py --write) -->",
               "<!-- COMPARE-MATRIX:END -->",
               lambda: matrix_markdown()),
}


def write_into_docs(which="matrix"):
    """Schreibt eine generierte Tabelle idempotent zwischen die Marker im Zieldokument."""
    rel, begin, end, gen = _TARGETS[which]
    path = os.path.join(ROOT, rel)
    text = open(path, encoding="utf-8").read()
    if begin not in text or end not in text:
        raise SystemExit("Marker fehlen in " + rel)
    pre, rest = text.split(begin, 1)
    _, post = rest.split(end, 1)
    body = gen()
    open(path, "w", encoding="utf-8").write(pre + begin + "\n\n" + body + "\n\n" + end + post)
    return {"geschrieben": rel, "zeilen": body.count("\n") + 1}
