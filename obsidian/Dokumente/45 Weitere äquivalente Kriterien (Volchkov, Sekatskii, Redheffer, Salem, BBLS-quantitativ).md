...
tags:
  ...
  - "kategorie/criterion"
  - "status/open"
  - "thema/baez-duarte-quantitative"
  - "thema/redheffer-matrix"
  - "thema/salem"
  - "thema/sekatskii"
  - "thema/volchkov"
quelle: docs/45_further_equivalent_criteria.md
---

> [!info] Navigation
> **Karte:** [[MOC M – Meta ∕ 'Bulletproof'-Schicht (Obstruktionen, Synthese, Verifikation)]] · **Kategorie:** Äquivalente Kriterien · **Status:** `OFFEN`
> **Zentrale Notiz:** [[Riemann-Wissensnetz]] · **Original:** `docs/45_further_equivalent_criteria.md`

# Weitere äquivalente Kriterien (Volchkov, Sekatskii, Redheffer, Salem, BBLS-quantitativ)

**Kategorie:** Äquivalente Kriterien (Ergänzung zu Dok. [[13 Nyman–Beurling-Kriterium & Báez-Duarte-Verschärfung|13]]–[[16 Mertens-Funktion & Riesz-Kriterium (Möbius-basierte Kriterien)|16]])
...
```
Dann gilt det(R_n) = M(n) (Mertens-Funktion, Dok. [[16 Mertens-Funktion & Riesz-Kriterium (Möbius-basierte Kriterien)|16]]!). Daher:
```
...
Über eine Integralgleichung vom Wiener–Tauberischen Typ: die Nicht-Verschwindung einer bestimmten Integraltransformierten (Faltungskern e^{σx}/(e^{e^x}+1)-artig) ist äquivalent zu ζ(σ+it) ≠ 0 auf einer vertikalen Geraden. Verbindet RH mit Vollständigkeit/Dichte (verwandt Nyman–Beurling, Dok. [[13 Nyman–Beurling-Kriterium & Báez-Duarte-Verschärfung|13]]).

### BBLS — quantitative Nyman–Beurling-Distanz (2000)
Mit d_N² = inf_{Polynom-Koeff.} ‖1 − Σ_{k≤N} c_k ρ_{1/k}‖²_{L²(0,1)} (vgl. Dok. [[13 Nyman–Beurling-Kriterium & Báez-Duarte-Verschärfung|13]]):
```
...
- Redheffer verbindet direkt zur Mertens-Funktion (Dok. [[16 Mertens-Funktion & Riesz-Kriterium (Möbius-basierte Kriterien)|16]]); Volchkov/Sekatskii bieten *Gleichheiten* (sensible Tests); BBLS gibt eine berechenbare Zielkonstante.
...
- **ist äquivalent zu** → [[Riemann-Vermutung (RH)]] — *Volchkov/Sekatskii/Redheffer/Salem/BBLS-Kriterien ⟺ RH.*

### Eingehende Relationen

- **ist äquivalent zu** ← [[16 Mertens-Funktion & Riesz-Kriterium (Möbius-basierte Kriterien)]] — *Redheffer-Matrix det(R_n)=M(n) verbindet Mertens und Matrix-Kriterium.*

### Belegte Aussagen (Claims)

- `[BEWIESEN]` [[claim-volchkov]] — RH ⟺ ∫₀^∞ (1−12t²)/(1+4t²)³·log|ζ(1/2+it)| dt = π(3−γ)/32.
- `[BEWIESEN]` [[claim-redheffer]] — RH ⟺ det(R_n)=O(n^{1/2+ε}), wobei det(R_n)=M(n).
