...
tags:
  ...
  - "thema/mertens-warning"
  - "thema/obstructions"
  - "thema/parity-problem"
  - "thema/skewes"
quelle: docs/35_obstructions_barriers.md
---

> [!info] Navigation
> **Karte:** [[MOC M – Meta ∕ 'Bulletproof'-Schicht (Obstruktionen, Synthese, Verifikation)]] · **Kategorie:** Obstruktionen · **Status:** `META`
> **Zentrale Notiz:** [[Riemann-Wissensnetz]] · **Original:** `docs/35_obstructions_barriers.md`
...
Dies ist die schärfste bekannte Obstruktion. Sie disqualifiziert sofort viele „elementare" und rein funktionentheoretische Beweisversuche (vgl. Dok. [[27 Weitere umstrittene, zurückgezogene & fehlerhafte Beweisbehauptungen|27]]).

## 2. Die Selberg-Klasse-Schranke
In der Selberg-Klasse (Dok. [[21 Verallgemeinerte, Große Riemann-Vermutung & Selberg-Klasse|21]]) wird die RH nur für Funktionen **mit Euler-Produkt** erwartet. Funktionen mit Grad 1 ohne Euler-Produkt (wie Davenport–Heilbronn) sind Gegenbeispiele. ⇒ Jeder Beweis muss zwischen „mit" und „ohne" Euler-Produkt unterscheiden können.

## 3. Das Paritätsproblem (Sieb-Methoden)
Klassische Siebmethoden (Brun, Selberg) können **prinzipiell** nicht zwischen Zahlen mit gerader und ungerader Anzahl von Primfaktoren unterscheiden (Paritätsbarriere, Selberg). Da die Möbius-Funktion μ(n) = (−1)^{Ω(n)} genau diese Parität misst und 1/ζ = Σ μ(n)/n^s, können reine Siebargumente die für die RH nötige Kontrolle über M(x) (Dok. [[16 Mertens-Funktion & Riesz-Kriterium (Möbius-basierte Kriterien)|16]]) nicht liefern.

## 4. Gründe zum Zweifeln (Ivić 2003) — Vorsicht vor „zu schöner" Evidenz
- **Mertens-Vermutung widerlegt** (Dok. [[16 Mertens-Funktion & Riesz-Kriterium (Möbius-basierte Kriterien)|16]]): |M(x)| < √x scheint bis 10^{14} zu gelten, ist aber falsch. Numerik täuscht.
...
- **S(T)-Wachstum:** Der Argumentterm S(T) (Dok. [[02 Riemann–von-Mangoldt-Formel und die explizite Formel|02]]) ist im Mittel klein, wird aber (unter RH) unbeschränkt — sehr hohe Nullstellen könnten unerwartetes Verhalten zeigen, das bei heutigen Höhen unsichtbar ist.
- Sehr nahe Nullstellenpaare (**Lehmer-Paare**, Dok. [[23 De-Bruijn–Newman-Konstante – Rodgers–Tao & Polymath15|23]]) zeigen, dass die RH (falls wahr) nur „knapp" gilt — keine komfortable Marge.

## 5. Warum spektrale Ansätze nicht „geschenkt" sind
- Ein Hilbert–Pólya-Operator (Dok. [[05 Die Hilbert–Pólya-Vermutung (spektraler Ansatz)|05]]) muss **kanonisch aus der Arithmetik** kommen; ein ad-hoc-Operator mit Spektrum {γ_n} zu „erfinden" beweist nichts (man kann zu jeder reellen Folge einen selbstadjungierten Operator angeben — zirkulär, wenn man die Realität schon annimmt). Genau das ist die Lücke bei Bender–Brody–Müller (Dok. [[09 Bender–Brody–Müller (2017) – PT-symmetrischer Hamiltonian für die Riemann-Nullstellen|09]]).
- Connes' Programm umgeht dies, indem die Positivität *unabhängig* gezeigt werden müsste — und genau das ist offen (Dok. [[10 Alain Connes – Spurformel & nichtkommutative Geometrie|10]]).
...
3. **Wird Positivität (Li/Weil/de Branges) angenommen oder bewiesen?** Annahme → zirkulär (Dok. [[14 Li-Kriterium, Bombieri–Lagarias & Weil-Positivität|14]], [[20 Louis de Branges – Hilberträume ganzer Funktionen (mehrfach gescheiterte Beweise)|20]]).
4. **Beruht die Evidenz nur auf endlicher Numerik?** → kein Beweis (Mertens, Skewes).
5. **Vertauscht der Beweis Limes/Summe über die nicht absolut konvergente Nullstellensumme?** → Fehler (Dok. [[27 Weitere umstrittene, zurückgezogene & fehlerhafte Beweisbehauptungen|27]]).
...
- **wird gestützt durch** ← [[16 Mertens-Funktion & Riesz-Kriterium (Möbius-basierte Kriterien)]] — *Widerlegte Mertens-Vermutung: numerische Evidenz täuscht (Warnung).*
- **wird gestützt durch** ← [[39 Cramér-Modell & probabilistische Heuristiken der Primzahlen]] — *Maier-Satz: probabilistisches Modell im Detail falsch (Warnung).*

### Belegte Aussagen (Claims)

- `[BEWIESEN]` [[claim-dh]] — Die Davenport–Heilbronn-Funktion (ζ-artig, OHNE Euler-Produkt) hat Nullstellen abseits Re=1/2.
